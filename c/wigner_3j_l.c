...
//     will contain `f(l1min+i), i=0, 1, ..., l1max+l1min`.
// ndim : int
                ...
                double* l1max_out, double* thrcof, int ndim)
{
    ...
    // constants
    const double zero = 0., eps = .01, one = 1., two = 2., three = 3.;
    ...
    if((fmod(l2+fabs(m2)+eps, one) >= eps+eps) ||
    ...
    *l1max_out = l1max = l2 + l3;
    ...
    if(l1min >= l1max-eps)
    {
        thrcof[0] = phase(fabs(l2+m2-l3+m3)+eps)/sqrt(l1min + l2 + l3 + one);
        return 0;
    }
